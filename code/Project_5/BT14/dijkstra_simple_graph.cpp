...
pair<vector<int>, vector<int>> dijkstra(vector<vector<pii>>& adj_list, int start) {
    int n = adj_list.size();
...
vector<vector<pii>> read_graph(string filename) {
...
int main() {
    ...
    vector<vector<pii>> adj_list = read_graph("simple_graph.inp");
    
    int start_vertex = 3;
    auto result = dijkstra(adj_list, start_vertex);
