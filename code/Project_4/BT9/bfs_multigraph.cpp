...
void read_multigraph_from_file(const string& filename, int& n, vector<vector<int>>& adj) {
    ifstream fin(filename);
    string line;
    int m;
    
    // Read number of vertices and edges, skip comments
...
int main() {
    string filename = "multigraph.inp";
    int n;
    vector<vector<int>> adj;
    read_multigraph_from_file(filename, n, adj);
    bfs(n, adj, 0);
    return 0;
}
